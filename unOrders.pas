...
begin
  lb1.Clear;
  if cb1.Checked then
    lb1.Items.Add('Незавершенные');
  if cb2.Checked then
    lb1.Items.Add('Завершенные');
  if cb3.Checked then
    lb1.Items.Add('Отвергнутые');
  if cb6.Checked then
    lb1.Items.Add('Основные средства');
  if cb7.Checked then
    lb1.Items.Add('Есть комиссионные');
  if cb8.Checked then
    lb1.Items.Add('Не полностью оплаченные');
  s := '';
  ...
  begin
    case cbx2.ItemIndex of
      1: s := 'Дата создания c ';
      2: s := 'Дата конструкции c ';
      3: s := 'Дата выполнения c ';
      4: s := 'Дата завершения c ';
      5: s := 'Дата последней отгрузки c ';
    end;
  ...
  if lcb1.KeyValue <> NULL then
    lb1.Items.Add('Производство: ' + qPRODNAME.AsString);
  if lcb2.KeyValue <> NULL then
    lb1.Items.Add('Тип продукции: ' + qTPRNAME.AsString);
  if lcb3.KeyValue <> NULL then
    lb1.Items.Add('Менеджер: ' + qMANFIO.AsString);
  if lcb4.KeyValue <> NULL then
    lb1.Items.Add('Клиент: ' + qAGENTSNAME.AsString);
  if lcb5.KeyValue <> NULL then
    lb1.Items.Add('Дизайнер: ' + qDEZFIO.AsString);
  if lcb6.KeyValue <> NULL then
    lb1.Items.Add('Конструктор: ' + qCONSTRFIO.AsString);
  if lcb7.KeyValue <> NULL then
    lb1.Items.Add('Выпускающий участок: ' + qPA_OUTNAME.AsString);
  if lcb8.KeyValue <> NULL then
    lb1.Items.Add('Склад готовой продукции: ' + qSTORENAME.AsString);
  pc1.ActivePage := ts1;
end;
...
begin
  { if not dmIS.dbDOC.Connected then
   try
     ...
     MsgInformation('Нет связи с архивом данных', '');
   end;     }
  lb1.Clear;
...
begin
  case qOrdersSTATUS.AsInteger of
    0: qOrdersNSTSTUS.AsString := 'Черновик';
    10: qOrdersNSTSTUS.AsString := 'Утвержден менеджером';
    20: qOrdersNSTSTUS.AsString := 'Утвержден руководителем';
    30: qOrdersNSTSTUS.AsString := 'Получен производством';
    40: qOrdersNSTSTUS.AsString := 'Утвержден в производство';
    45: qOrdersNSTSTUS.AsString := 'Принят конструктором';
    50: qOrdersNSTSTUS.AsString := 'Разработана технология';
    60: qOrdersNSTSTUS.AsString := 'Утверждена технология';
    70: qOrdersNSTSTUS.AsString := 'Принят в изготовление';
    100: qOrdersNSTSTUS.AsString := 'Изготовлен полностью';
    110: qOrdersNSTSTUS.AsString := 'Вывезен полностью';
    120: qOrdersNSTSTUS.AsString := 'Выписан полностью';
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Old_DocBody(iddr, 0, ''); // переход на новую базу документов
...
begin
  ...
  begin
    ...
    try
      ...
      if not Assigned(fmAll) then
        Application.CreateForm(TfmAll, fmAll);
      Old_DocBody(d_id, 0, ''); // переход нановую базу документов
...
begin
  ...
  begin
    showmessage('Нельзя изменить клиента!');
    exit;
