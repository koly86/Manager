...
type
  ...
  private
    ...
    procedure SetParam(fname : string; id_param : integer); //записывает парметры опер в базу
...
begin
 ed_name_t:=ed_name.text;
  if   ed_name_t = '' then
     Begin
       showmessage('Введите наименование новой позиции');
       ed_name.SetFocus;
       exit;
     end else
 ...
 { if name = ed_name_t then
      begin
        showmessage('Повторяющейся тип изделия');
        exit;
      end else  }


  if code = 1 then   //Добавить позицию
  begin
    with dmIs.qIM, dmIS.qIM.SQL do
    begin
      Close;
   ...
   if   ed_name.Text = '' then
     Begin
       showmessage('Введите наименование изделия');
       ed_name.SetFocus;
     exit;
     end else


  if code = 0 then //Копировать тип изделия
  begin
...
begin
  ...
  begin
    ...
    if not dmIS.qIN.IsEmpty then
    begin
      MsgError('Данный тип изделия используется', 'Проверка перед удалением');
      Exit;
    end;
  end;
  with dmIS.qIM, dmIS.qIM.SQL do
...
begin
  ...
  begin
    tb8.ImageIndex := 24;
    tb8.Hint := 'Свернуть список';
    dbtries.FullExpand;
  end
  else
  begin
    tb8.ImageIndex := 23;
    tb8.Hint := 'Развернуть список';
    dbtries.BuildTree;
  end;
end;
...
begin
  code := 0;   //Копировать тип изделия
  gb1.Show;
  b1.Caption := 'Копировать';
  ed_name.SetFocus;
end;
...
begin
  ...
  begin
    ...
    if not dmIS.qIN.IsEmpty then
    begin
      MsgError('Проверка перед удалением',
        'Данное примечание используется в операциях');
      Exit;
    end;
  end;
  with dmIs.qIM, dmIS.qIM.SQL do
...
begin
  if tb12.Down then
  begin
    tb12.Hint := 'Во все типы изделий';
    s_all := 1;
  end
  else
  begin
    tb12.Hint := 'По конкретному типу изделия';
    s_all := 0;
  end;
end;
