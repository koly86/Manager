object fmTr: TfmTr
  Left = 231
  Top = 30
  Width = 945
  Height = 680
  Caption = #1048#1079#1076#1077#1083#1080#1103
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Splitter2: TSplitter
    Left = 276
    Top = 0
    Height = 605
    Align = alRight
  end
  object Panel1: TPanel
    Left = 0
    Top = 605
    Width = 937
    Height = 41
    Align = alBottom
    TabOrder = 0
    object SpeedButton1: TSpeedButton
      Left = 360
      Top = 8
      Width = 23
      Height = 22
      Glyph.Data = {
        
        777777770FFFBFF08777777770FBFFBF08777777700000000077}
      Visible = False
      OnClick = SpeedButton1Click
    end
    object SpeedButton2: TSpeedButton
      Left = 392
      Top = 8
      Width = 23
      Height = 22
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      OnClick = SpeedButton2Click
    end
    object SpeedButton3: TSpeedButton
      Left = 424
      Top = 8
      Width = 23
      Height = 22
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      OnClick = SpeedButton3Click
    end
    object Label1: TLabel
      Left = 8
      Top = 16
      Width = 32
      Height = 13
      Caption = #1055#1086#1080#1089#1082
    end
    object edFnd: TEdit
      Left = 48
      Top = 13
      Width = 121
      Height = 21
      TabOrder = 0
    end
  end
  object GroupBox1: TGroupBox
    Left = 279
    Top = 0
    Width = 449
    Height = 605
    Align = alRight
    Caption = #1057#1086#1089#1090#1086#1103#1085#1080#1077' '#1089#1082#1083#1072#1076#1072' '#1043#1055
    TabOrder = 1
    object dbgGPState: TDBGridEh
      Left = 2
      Top = 15
      Width = 445
      Height = 588
      Align = alClient
      DataSource = dsGpState
      Flat = True
      FooterColor = clWindow
      FooterFont.Charset = DEFAULT_CHARSET
      FooterFont.Color = clWindowText
      FooterFont.Height = -11
      FooterFont.Name = 'MS Sans Serif'
      FooterFont.Style = []
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      UseMultiTitle = True
      Columns = <
        item
          EditButtons = <>
          FieldName = 'ID_P_AREA'
          Footers = <>
        end
        item
          EditButtons = <>
          FieldName = 'ANAME'
          Footers = <>
          Width = 236
        end
        item
          EditButtons = <>
          FieldName = 'OBJ_NAME'
          Footers = <>
          Width = 112
        end
        item
          EditButtons = <>
          FieldName = 'VNAME'
          Footers = <>
          Width = 222
        end
        item
          EditButtons = <>
          FieldName = 'ID_BATCH'
          Footers = <>
        end
        item
          EditButtons = <>
          FieldName = 'ID_ORDER'
          Footers = <>
        end
        item
          EditButtons = <>
          FieldName = 'AMOUNT'
          Footers = <>
        end
        item
          EditButtons = <>
          FieldName = 'PRICE'
          Footers = <>
        end
        item
          EditButtons = <>
          FieldName = 'SUMMA'
          Footers = <>
        end
        item
          EditButtons = <>
          FieldName = 'FREE'
          Footers = <>
        end
        item
          EditButtons = <>
          FieldName = 'REZERV'
          Footers = <>
        end
        item
          EditButtons = <>
          FieldName = 'REZERVPONAKL'
          Footers = <>
        end>
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 276
    Height = 605
    Align = alClient
    Caption = 'Panel2'
    TabOrder = 2
    object Splitter1: TSplitter
      Left = 1
      Top = 481
      Width = 274
      Height = 3
      Cursor = crVSplit
      Align = alBottom
    end
    object dbTr: TDBS_TreeView
      Left = 1
      Top = 1
      Width = 274
      Height = 480
      Align = alClient
      Indent = 19
      TabOrder = 0
      OnChange = dbTrChange
      OnDblClick = dbTrDblClick
      Database = dmIS.dbIS
      Transaction = dmIS.mT
      RootId = 0
      TableName = 'GEt_TREE_PRODS'
      FieldId = 'ID_ITEM'
      FieldParentId = 'ID_PARENT'
      FieldName = 'NAME'
      FieldState = 'LOW'
      FieldSort = 'NAME'
    end
    object dbgVer: TDBGridEh
      Left = 1
      Top = 484
      Width = 274
      Height = 120
      Align = alBottom
      DataSource = dsVer
      Flat = True
      FooterColor = clWindow
      FooterFont.Charset = DEFAULT_CHARSET
      FooterFont.Color = clWindowText
      FooterFont.Height = -11
      FooterFont.Name = 'MS Sans Serif'
      FooterFont.Style = []
      TabOrder = 1
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      UseMultiTitle = True
      Columns = <
        item
          EditButtons = <>
          FieldName = 'ID_VERSION'
          Footers = <>
        end
        item
          EditButtons = <>
          FieldName = 'NAME'
          Footers = <>
          Width = 373
        end>
    end
  end
  object Panel3: TPanel
    Left = 728
    Top = 0
    Width = 209
    Height = 605
    Align = alRight
    TabOrder = 3
    object GroupBox2: TGroupBox
      Left = 1
      Top = 1
      Width = 207
      Height = 207
      Align = alClient
      Caption = #1060#1086#1088#1084#1080#1088#1086#1074#1072#1085#1080#1077' '#1085#1072#1082#1083#1072#1076#1085#1086#1081
      TabOrder = 0
      object Label2: TLabel
        Left = 8
        Top = 32
        Width = 26
        Height = 13
        Caption = #1062#1077#1085#1072
      end
      object Label3: TLabel
        Left = 8
        Top = 64
        Width = 34
        Height = 13
        Caption = #1050#1086#1083'-'#1074#1086
      end
      object Label4: TLabel
        Left = 8
        Top = 96
        Width = 34
        Height = 13
        Caption = #1057#1091#1084#1084#1072
      end
      object edPrice: TEdit
        Left = 48
        Top = 32
        Width = 121
        Height = 21
        ReadOnly = True
        TabOrder = 0
      end
      object edAmount: TEdit
        Left = 48
        Top = 64
        Width = 121
        Height = 21
        TabOrder = 1
      end
      object edSumm: TEdit
        Left = 48
        Top = 96
        Width = 121
        Height = 21
        TabOrder = 2
      end
      object BitBtn1: TBitBtn
        Left = 4
        Top = 144
        Width = 197
        Height = 41
        Caption = #1047#1072#1087#1080#1089#1072#1090#1100' '#1087#1086#1079#1080#1094#1080#1102' '#1074' '#1085#1072#1082#1083#1072#1076#1085#1091#1102
        Default = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        WordWrap = True
        OnClick = BitBtn1Click
        Glyph.Data = {
          
          0000}
        NumGlyphs = 2
      end
    end
    object GroupBox3: TGroupBox
      Left = 1
      Top = 208
      Width = 207
      Height = 396
      Align = alBottom
      Caption = #1057#1088#1077#1076#1085#1077#1077' '#1074' '#1089#1087#1077#1094#1080#1092#1080#1082#1072#1094#1080#1080
      TabOrder = 1
      object Label5: TLabel
        Left = 16
        Top = 24
        Width = 26
        Height = 13
        Caption = #1062#1077#1085#1072
      end
      object Label6: TLabel
        Left = 16
        Top = 56
        Width = 34
        Height = 13
        Caption = #1050#1086#1083'-'#1074#1086
      end
      object Label7: TLabel
        Left = 16
        Top = 96
        Width = 34
        Height = 13
        Caption = #1057#1091#1084#1084#1072
      end
      object Label8: TLabel
        Left = 16
        Top = 128
        Width = 44
        Height = 13
        Caption = #1053#1072#1094#1077#1085#1082#1072
      end
      object Label9: TLabel
        Left = 16
        Top = 152
        Width = 55
        Height = 13
        Caption = #1057#1090#1086#1080#1084#1086#1089#1090#1100
      end
      object edMedprice: TEdit
        Left = 80
        Top = 24
        Width = 121
        Height = 21
        ReadOnly = True
        TabOrder = 0
      end
      object edmedAmount: TEdit
        Left = 80
        Top = 56
        Width = 121
        Height = 21
        ReadOnly = True
        TabOrder = 1
      end
      object edMedSumma: TEdit
        Left = 80
        Top = 88
        Width = 121
        Height = 21
        ReadOnly = True
        TabOrder = 2
      end
      object edMedProfit: TEdit
        Left = 80
        Top = 120
        Width = 121
        Height = 21
        TabOrder = 3
        OnKeyDown = edMedProfitKeyDown
      end
      object edMedprofSumm: TEdit
        Left = 80
        Top = 152
        Width = 121
        Height = 21
        ReadOnly = True
        TabOrder = 4
      end
      object BitBtn2: TBitBtn
        Left = 1
        Top = 344
        Width = 204
        Height = 33
        Caption = #1047#1072#1087#1080#1089#1072#1090#1100' '#1074' '#1089#1087#1077#1094#1080#1092#1080#1082#1072#1094#1080#1102' '#1079#1072#1082#1072#1079#1072
        TabOrder = 5
        Kind = bkOK
      end
    end
  end
  object qIns: TIBQuery
    Database = dmIS.dbIS
    Transaction = dmIS.mT
    Left = 240
    Top = 104
  end
  object qVEr: TIBQuery
    Database = dmIS.dbIS
    Transaction = dmIS.mT
    BeforeOpen = qVErBeforeOpen
    SQL.Strings = (
      
        'select id_version, name from product_versions where id_item = :i' +
        'd_item')
    Left = 225
    Top = 267
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'id_item'
        ParamType = ptUnknown
      end>
    object qVErID_VERSION: TIntegerField
      DisplayLabel = #1056#1077#1075'. '#8470
      FieldName = 'ID_VERSION'
      Origin = '"PRODUCT_VERSIONS"."ID_VERSION"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qVErNAME: TIBStringField
      DisplayLabel = #1042#1077#1088#1089#1080#1103
      FieldName = 'NAME'
      Origin = '"PRODUCT_VERSIONS"."NAME"'
      Size = 128
    end
  end
  object dsVer: TDataSource
    DataSet = qVEr
    Left = 249
    Top = 347
  end
  object qGpState: TIBQuery
    Database = dmIS.dbIS
    Transaction = dmIS.mT
    AfterScroll = qGpStateAfterScroll
    BeforeOpen = qGpStateBeforeOpen
    DataSource = dsVer
    SQL.Strings = (
      
        'select a.id_p_area, a.name aname, coalesce( b.obj_name, i.name) ' +
        'obj_name  ,'
      
        '     v.name vname, s.id_batch, s.id_order_pos, p.id_order,  p.id' +
        '_product,'
      '     z.summa/z.amount price, sum(z.summa) summa,'
      '    sum(s.amount) - sum(z.amount) - sum(y.amount) free,'
      '    sum(s.amount) amount, sum(y.amount) rezerv,'
      '    sum(z.amount) rezervponakl'
      '    from p_area_states s'
      '    join prod_areas a on a.id_p_area = s.id_p_area'
      
        '    left join project_ords_pos p on p.id_order_pos = s.id_order_' +
        'pos'
      '    join product_objects b on b.id_object = s.id_object'
      '    join product_versions v on v.id_version = b.id_version'
      '    left join items i on i.id_item = b.id_item'
      '    left join pa_bill_pos y on y.id_batch = s.id_batch'
      '    left join p_area_bills b1 on b1.id_pa_bill = y.id_pa_bill'
      '      and b1.pa_bill_type = 122    '
      '    left join pa_bill_pos z on z.id_batch = s.id_batch'
      '    left join p_area_bills b2 on b2.id_pa_bill = z.id_pa_bill'
      '      and b2.pa_bill_type in(110,106,101)'
      '    where s.id_object in'
      
        '     (select id_object from product_objects where id_version = 5' +
        '4466)'
      '    and a.id_patype = 10 and a.d_close is null'
      '    and s.id_object_type = 4'
      ''
      '    group by 1,2,3,4,5, 6,7,8,9')
    Left = 449
    Top = 175
    object qGpStateID_P_AREA: TIntegerField
      DisplayLabel = #1050#1086#1076' '#1089#1082#1083#1072#1076#1072
      FieldName = 'ID_P_AREA'
      Origin = '"PROD_AREAS"."ID_P_AREA"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qGpStateANAME: TIBStringField
      DisplayLabel = #1057#1082#1083#1072#1076
      FieldName = 'ANAME'
      ProviderFlags = []
      Size = 80
    end
    object qGpStateOBJ_NAME: TIBStringField
      DisplayLabel = #1044#1077#1090#1072#1083#1100
      FieldName = 'OBJ_NAME'
      Origin = '"PRODUCT_OBJECTS"."OBJ_NAME"'
      Size = 128
    end
    object qGpStateVNAME: TIBStringField
      DisplayLabel = #1042#1077#1088#1089#1080#1103
      FieldName = 'VNAME'
      ProviderFlags = []
      Size = 128
    end
    object qGpStateID_BATCH: TIntegerField
      DisplayLabel = #1055#1072#1088#1090#1080#1103
      FieldName = 'ID_BATCH'
      Origin = '"P_AREA_STATES"."ID_BATCH"'
    end
    object qGpStateID_ORDER_POS: TIntegerField
      FieldName = 'ID_ORDER_POS'
      Origin = '"P_AREA_STATES"."ID_ORDER_POS"'
    end
    object qGpStateID_ORDER: TIntegerField
      DisplayLabel = #1056#1077#1079#1077#1088#1074' '#1085#1072' '#1079#1072#1082#1072#1079
      FieldName = 'ID_ORDER'
      Origin = '"PROJECT_ORDS_POS"."ID_ORDER"'
    end
    object qGpStateID_PRODUCT: TIntegerField
      FieldName = 'ID_PRODUCT'
      Origin = '"PROJECT_ORDS_POS"."ID_PRODUCT"'
    end
    object qGpStateAMOUNT: TIBBCDField
      DisplayLabel = #1050#1086#1083'-'#1074#1086
      FieldName = 'AMOUNT'
      ProviderFlags = []
      DisplayFormat = '### ### ###.000'
      Precision = 18
      Size = 4
    end
    object qGpStatePRICE: TFMTBCDField
      DisplayLabel = #1062#1077#1085#1072
      FieldName = 'PRICE'
      ProviderFlags = []
      DisplayFormat = '### ### ###.000'
      Precision = 18
      Size = 8
    end
    object qGpStateSUMMA: TIBBCDField
      DisplayLabel = #1057#1091#1084#1084#1072
      FieldName = 'SUMMA'
      ProviderFlags = []
      Precision = 18
      Size = 4
    end
    object qGpStateFREE: TIBBCDField
      DisplayLabel = #1057#1074#1086#1073#1086#1076#1085#1086
      FieldName = 'FREE'
      ProviderFlags = []
      Precision = 18
      Size = 4
    end
    object qGpStateREZERV: TIBBCDField
      DisplayLabel = #1056#1077#1079#1077#1088#1074' '#1079#1072#1082#1072#1079#1072' '#1085#1072' '#1087#1088#1086#1076#1072#1078#1091
      FieldName = 'REZERV'
      ProviderFlags = []
      Precision = 18
      Size = 4
    end
    object qGpStateREZERVPONAKL: TIBBCDField
      DisplayLabel = #1056#1077#1079#1077#1088#1074' '#1087#1086' '#1085#1072#1082#1083#1072#1076#1085#1099#1084
      FieldName = 'REZERVPONAKL'
      ProviderFlags = []
      Precision = 18
      Size = 4
    end
  end
  object dsGpState: TDataSource
    DataSet = qGpState
    Left = 545
    Top = 167
  end
  object qMedPos: TIBQuery
    Database = dmIS.dbIS
    Transaction = dmIS.mT
    AfterOpen = qMedPosAfterOpen
    BeforeOpen = qMedPosBeforeOpen
    SQL.Strings = (
      ' select avg(s.amount) med_am, avg(s.summa) med_sum,'
      '    avg(s.summa)/avg(s.amount) med_price,'
      
        '    avg(s.summa)*(1 + cast(:prof as double precision )/100) med_' +
        'prof_price'
      '    from pa_bill_pos s'
      '    join p_area_bills b on b.id_pa_bill = s.id_pa_bill'
      '    where s.id_order = :id_order and s.id_object = :id_object')
    Left = 503
    Top = 312
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'prof'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'id_order'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'id_object'
        ParamType = ptUnknown
      end>
    object qMedPosMED_AM: TIBBCDField
      FieldName = 'MED_AM'
      ProviderFlags = []
      Precision = 18
      Size = 4
    end
    object qMedPosMED_SUM: TIBBCDField
      FieldName = 'MED_SUM'
      ProviderFlags = []
      Precision = 18
      Size = 4
    end
    object qMedPosMED_PRICE: TFMTBCDField
      FieldName = 'MED_PRICE'
      ProviderFlags = []
      Precision = 18
      Size = 8
    end
    object qMedPosMED_PROF_PRICE: TFloatField
      FieldName = 'MED_PROF_PRICE'
      ProviderFlags = []
    end
  end
  object dsMedPos: TDataSource
    DataSet = qMedPos
    Left = 503
    Top = 400
  end
end
