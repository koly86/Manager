...
begin
  ...
  begin
    ...
    Add('case when p.status =7  then ''' + 'Прогноз' + ''' else');
    Add('case when p.status =20  then ''' + 'Утвержден руководителем' +
      ''' else');
    Add('case when p.status =30  then ''' + 'Получен производством' + '''else');
    Add('case when p.status =40  then ''' + 'Утвержден в производство' +
      ''' else');
    Add('case when p.status =45  then ''' + 'Принят конструктором' + ''' else');
    Add('case when p.status =50  then ''' + 'Разработана технология' +
      ''' else');
    Add('case when p.status =60  then ''' + 'Технология утверждена' +
      ''' else ');
    Add('case when p.status =70  then ''' + 'Принят в изготовление' +
      ''' else');
    Add('case when p.status =100 then ''' + 'Изготовление завершено' +
      ''' else');
...
begin
  ...
  Old_DocBody(dmIS.qDOCID_DRAFT.AsInteger, 0, ''); // переход на новую базу документов
...
begin
  ...
  if MsgQuestion(SysVars.NReg + ', Вы действительно хотите удалить ' +
    ...
    dmIS.qDOCNAME.AsString + ' ?', 'Удаление приложения') = id_yes then
  begin
...
begin
  ...
  begin
    ...
    begin
      ID_I := dmIS.qDOCID_DRAFT.Value; //поле ID_DRAFT is not null
...
begin
 ...
 //   MsgError('Невоспроизводимый графический формат',
...
begin
  if dmIS.qDOCID_DRAFT.IsNull then
    exit;
   Old_DocBody(dmIS.qDOCID_DRAFT.AsInteger, 4, ''); // переход на новую базу документов
...
begin
  ...
  if dmIS.qDOC.IsEmpty then
  begin
    MsgInformation('Не задан документ-основание для заявки',
      'Проверка документа');
    exit;
  end;
  if dmIS.qDOCID_DRAFT.IsNull then
...
begin
  ...
  if not Assigned(fmAll) then
    Application.CreateForm(TfmAll, fmAll);
 Old_DocBody(dmIS.qEstReqID_DOC.AsInteger, 0 ,''); // переход на новую версию базы документов
