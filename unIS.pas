...
//  SysVars.RegPost := dmIS.qIN.FieldByName('IDPOST').AsInteger;
 ...
 // SysVars.RegAccess := qIN.FieldByName('ACCESS_KEY').AsInteger;
  ...
  fmMain.S19 := ReadSysIntParam(dmIS.qIM, 19); //авто расчет себестоимости
  fmMain.S35 := ReadSysIntParam(dmIS.qIM, 35); //учитывать срок жизни сметы
  fmMain.S36 := ReadSysIntParam(dmIS.qIM, 36); //срок жизни сметы
  fmMain.S51 := ReadSysIntParam(dmIS.qIM, 51); //бренд обязателен?
  fmMain.S59 := ReadSysIntParam(dmIS.qIM, 59); //бренд обязателен?
...
begin
  case dmIS.qOBJOBJ_TYPE.AsInteger of
    0: dmIS.qOBJnTYPE.AsString := 'Деталь';
    1: dmIS.qOBJnTYPE.AsString := 'Комплектующая';
    2: dmIS.qOBJnTYPE.AsString := 'Материал';
    3: dmIS.qOBJnTYPE.AsString := 'Узел';
    4: dmIS.qOBJnTYPE.AsString := 'Изделие';
  end;
end;
...
begin
  case dmIS.qFILL_DF_TYPE.AsInteger of
    0: dmIS.qFILL_DnFILL.AsString := 'Отклонен';
    1: dmIS.qFILL_DnFILL.AsString := 'Подписан';
  end;
end;
...
begin
  ...
  0: qP_ORDSnSTAT.AsString := 'Новый';
  10: qP_ORDSnSTAT.AsString := 'Утвержден менеджером';
  20: qP_ORDSnSTAT.AsString := 'Утвержден руководителем';
  30: qP_ORDSnSTAT.AsString := 'Получен производством';
  40: qP_ORDSnSTAT.AsString := 'Утвержден в производство';
  45: qP_ORDSnSTAT.AsString := 'Принят конструктором';
  50: qP_ORDSnSTAT.AsString := 'Разработана технология';
  60: qP_ORDSnSTAT.AsString := 'Технология утверждена';
  70: qP_ORDSnSTAT.AsString := 'Принят в изготовление';
  100: qP_ORDSnSTAT.AsString := 'Изготовление завершено';
  110: qP_ORDSnSTAT.AsString := 'Вывезен полностью';
  120: qP_ORDSnSTAT.AsString := 'Получен заказчиком';
  200: qP_ORDSnSTAT.AsString := 'Отклонен';
  end;
